...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--foodji-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--foodji-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--foodji-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ foodji TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--foodji-${e(key)}`, value], [`--foodji-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`foodji.customProps.background.${colorScheme}`);
            const foreground = theme(`foodji.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--foodji-${e(key)}`, value], [`--foodji-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--foodji-${e(key)}`, value], [`--foodji-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
