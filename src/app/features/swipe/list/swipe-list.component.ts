...
export class SwipeListComponent implements OnInit {
  ...
  public cdkDragStarted(event: CdkDragStart): void {
    this.renderer.removeClass(
      this.getCardWrapper(event),
      ANIMATION_TRANSFORM_CSS_CLASS
    );
    this.renderer.removeClass(
        ...
        () =>
          this.store.dispatch(
            foodActions.likeFood({
              food: {
                id: foodId,
                changes: { liked: direction === 1 ? true : false },
              },
            })
          ),
        TRANSITION_TIME
  ...
  public evaluateFood(foodId: string, like: boolean) {
    ...
    setTimeout(
      () =>
        this.store.dispatch(
          foodActions.likeFood({
            food: { id: foodId, changes: { liked: like } },
          })
        ),
      TRANSITION_TIME
    );
  ...
  private animateElementOutOfScreen(element: Element, direction: number): void {
    this.renderer.setStyle(
      element,
      'transform',
      `translate3d(${130 * direction}%, 0px, 0px)`
    );
    this.renderer.setStyle(
      element.querySelector('.example-box'),
      'transform',
      `rotate(${20 * direction}deg)`
    );
  }
